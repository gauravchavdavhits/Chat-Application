      ...
      <div className="chat-header-bar">
        ...
        {selectedTarget && (
          <div className="chat-header-right" style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            {/* Voice & Video Call Buttons (Direct 1-on-1 and Group calls) */}
            <button 
              className="icon-btn" 
              onClick={() => {
                if (callUser && selectedTarget) {
                  const targetName = 'members' in selectedTarget ? selectedTarget.name : selectedTarget.username;
                  const targetAvatar = selectedTarget.avatar;
                  callUser(selectedTarget._id, false, targetName, targetAvatar);
                }
              }}
              title={'members' in selectedTarget ? "Start Group Voice Call" : "Voice Call"}
            >
            ...
            <button 
              ...
              onClick={() => {
                if (callUser && selectedTarget) {
                  const targetName = 'members' in selectedTarget ? selectedTarget.name : selectedTarget.username;
                  const targetAvatar = selectedTarget.avatar;
                  callUser(selectedTarget._id, true, targetName, targetAvatar);
                }
              }}
              title={'members' in selectedTarget ? "Start Group Video Call" : "Video Call"}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
